                      ...
                      GApplicationCommandLine *command_line)
{
    ...
    args = g_application_command_line_get_arguments (command_line, &size);

    if (size > 1 && g_strcmp0 (args[1], "--quit") == 0)
    {
        if (watcher->owner_id != 0)
        {
            g_debug ("Received quit command, shutting down XApp StatusNotifierWatcher");
            g_application_quit (application);
        }
    }
...
main (int argc, char **argv)
{
    ...
    g_application_add_main_option (G_APPLICATION (watcher),
                                   "quit", 'q',
                                   G_OPTION_FLAG_NONE,
                                   G_OPTION_ARG_NONE,
